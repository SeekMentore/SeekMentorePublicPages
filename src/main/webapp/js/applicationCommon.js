...
function submitFormResult() {
	...
	var xmlhttp = new XMLHttpRequest();
	  ...
	  xmlhttp.open("GET", "\student-result.xml", true);

//	  xmlhttp.open("GET", "C:\Users\JIGYASA\Documents\GitHub\SeekMentorePublicPages\src\main\webapp\js\student-result.xml", true);
	  xmlhttp.send();
	}
function showresult(xml) {
	var x, i, xmlDoc, txt, name, classId, phoneNo, address, marks, learningStyle, suggestion;
	xmlDoc = xml.responseXML;
	x = xmlDoc.getElementsByTagName("student");
	var id_name = document.getElementById('name');
	var id_class = document.getElementById('student-grade-big-screen');
	var id_contact = document.getElementById('contact-number');
	var table;	
	for( i=0; i< x.length;i++)
	{
		if(x[i].getElementsByTagName("name")[0].childNodes[0].nodeValue == id_name && x[i].getElementsByTagName("class")[0].childNodes[0].nodeValue ==  id_class && id_contact == x[i].getElementsByTagName("phone-no")[0].childNodes[0].nodeValue )
		{
			table="<tr><td>";
			table += x[i].getElementsByTagName("name")[0].childNodes[0].nodeValue + "</td><td>";
			table += x[i].getElementsByTagName("class")[0].childNodes[0].nodeValue + "</td><td>";
			table += x[i].getElementsByTagName("phone-no")[0].childNodes[0].nodeValue + "</td><td>";
			table += x[i].getElementsByTagName("marks")[0].childNodes[0].nodeValue + "</td><td>";
			table += x[i].getElementsByTagName("learning-style")[0].childNodes[0].nodeValue + "</td><td>";
			table += x[i].getElementsByTagName("suggestion")[0].childNodes[0].nodeValue + "</td></tr>";
	}
